...
type BJsTcpqqjl struct {
	...
	F_DT_ZUIXSJ   time.Time //`F_DT_ZUIXSJ`   '最新时间',
...
func TcpSendRecord(record BJsTcpqqjl) error {
	...
	//插入
	_, err := session.Insert(tcprecord)
	if err != nil {
...
func TcpResponseRecord(resRecord BJsTcpydjl) error {
	xorm := database.XormClient
	session := TransactionBegin(xorm)
